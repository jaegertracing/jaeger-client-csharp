...
namespace Jaeger.Encoders.Thrift.Internal
{
    internal static class JaegerThriftSpanConverter
    {
        public static Jaeger.Thrift.Span ConvertSpan(Span span)
        {
            ...
            var thriftSpan = new Jaeger.Thrift.Span(
                context.TraceId.Low,
        ...
        public static Jaeger.Thrift.Process ConvertProcess(Span span)
        {
            var tracer = span.Tracer;
            return new Jaeger.Thrift.Process
            {
        ...
        public static Jaeger.Thrift.Batch ConvertBatch(IEncodedProcess process, IEnumerable<IEncodedSpan> spans)
        {
            var encProcess = (ThriftProcess)process;
            var encSpans = spans.Cast<ThriftSpan>();

            return new Jaeger.Thrift.Batch
            {
                Process = encProcess.Process,
                Spans = encSpans.Select(s => s.Span).ToList()
            };
        }
